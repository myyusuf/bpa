define(["bpaObservable", "jqxbuttons", "jqxinput", "jqxvalidator", "jqxcombobox", "jqxwindow"], function (Observable) {
	...
	var InboxDetail = function(container, options){
		
		var _self = this;
		
		var _options = options || {};
		
		var _inboxTask = _options.inboxTask || {};
		...
		var _randomId = BPA.Util.getRandomId("inboxDetail");
        
		var _editWindow = $('<div id="inboxDetailWindow"></div>');
		var _windowHeader = $('<div style="height: 18px; padding: 5px; padding-top: 3px; padding-bottom: 7px;"><table><tr><td><img src="resources/images/application-dialog.png" alt="" style="margin-right: 1px" /></td><td valign="center"><span style="font-weight: bold">Inbox Task Detail</span></td></tr></table></div>');
		...
		var _idInput = $('<span style="font-weight: bold;">'+ _inboxTask.id +'</span>');
		...
		var _nameInput = $('<span style="font-weight: bold;">'+ _inboxTask.name +'</span>');
		...
		var _variables = _inboxTask.variables || [];
		...
		var _saveButton = $('<input type="button" value="Complete" style="margin-right: 5px; margin-top: 5px;"/>');
        ...
        var _completeTask = function(){
        	
        	var _savedData = {};
        	
        	_savedData.id = _inboxTask.id;
        	
        	Observable.prototype.publish.call(_self, _savedData, "onCompleteTask");
        }
	...
	inheritPrototype(InboxDetail, Observable);
